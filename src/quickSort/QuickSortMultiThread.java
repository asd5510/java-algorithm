...
public class QuickSortMultiThread {
    ...
    public static void sort(final int a[], final int low, final int high) {
        ...
        final int imm = i - 1;
        if (low < imm) {
            if (QuickSortMultiThread.THREADCOUNT.get() > 0 && imm - low > S) {
                Thread t = new Thread() {
                    {
                        QuickSortMultiThread.THREADCOUNT.decrementAndGet();
                    }
 
                    @Override
                    public void run() {
                        sort(a, low, imm);
                        QuickSortMultiThread.THREADCOUNT.addAndGet(1);
                    }
                };
                t.start();
                threads.add(t);
        ...
        final int ipp = i + 1;
        if (high > ipp) {
            if (QuickSortMultiThread.THREADCOUNT.get() > 0 && high - ipp > S) {
                Thread t = new Thread() {
                    {
                        QuickSortMultiThread.THREADCOUNT.decrementAndGet();
                    }
